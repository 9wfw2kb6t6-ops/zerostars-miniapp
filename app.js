...
document.getElementById("upgradeBtn").onclick = () => {
  ...
  game.coins -= cost; game.power++; game.dailyMissions.upgrade1 = true; update();
};
document.getElementById("minerBtn").onclick = () => {
  const cost = (game.miners + 1) * 100;
  if (game.coins < cost) return alert("Not enough Stars");
  game.coins -= cost; game.miners++; update();
};
document.getElementById("dailyBtn").onclick = () => {
  const today = new Date().toDateString();
  if (localStorage.getItem("zs_daily") === today) return alert("Already Claimed");
  localStorage.setItem("zs_daily", today); game.coins += 500; game.dailyMissions.dailyReward = true; update();
};
document.getElementById("boostBtn").onclick = () => { if (game.coins < 500) return alert("Need 500 Stars"); game.coins -= 500; game.xpBoost = true; update(); };
document.getElementById("energyBoostBtn").onclick = () => { if (game.coins < 300) return alert("Need 300 Stars"); game.coins -= 300; game.energy = 100; update(); };
document.getElementById("doubleCoinsBtn").onclick = () => { if (game.coins < 1000) return alert("Need 1000 Stars"); game.coins -= 1000; game.doubleCoins = true; update(); };
...
function claimTask(id) {
  if (id === 1 && !game.tasks.task1 && game.coins >= 100) { game.tasks.task1 = true; game.coins += 50; }
