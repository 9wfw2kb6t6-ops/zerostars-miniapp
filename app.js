...
function claimTelegramTask(){
  if(!game.tasks.telegram){ game.tasks.telegram=true; game.coins+=500; update(); }
}
window.claimTelegramTask = claimTelegramTask;

function claimTwitterTask(){
  if(!game.tasks.twitter){ game.tasks.twitter=true; game.coins+=500; update(); }
}
...
async function updateLeaderboard(){
  ...
  }catch(err){ console.error(err); board.innerHTML="Failed to load leaderboard."; }
}
